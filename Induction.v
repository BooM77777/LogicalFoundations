...
Proof.
  ...
  induction n as [|n' H].
  - simpl. reflexivity.
  - rewrite -> H. simpl. rewrite -> negb_involutive. reflexivity.
Qed.
...
Proof.
  ...
  rewrite add_assoc.
  rewrite add_assoc.
  assert ( m + n = n + m ). { rewrite add_comm. reflexivity. }
  rewrite H.
  reflexivity.
Qed.
...
Theorem mult_n_Sm  : forall n m : nat,
  ...
  - 
  {
    simpl. rewrite <- H. rewrite plus_n_Sm. rewrite plus_n_Sm. simpl. rewrite add_comm. 
    assert (H': forall a b c, a + (b + c) = b + (c + a)).
      { 
        intros a b c.
        rewrite add_shuffle3.
        assert (H'': a + c = c+ a). {rewrite add_comm. reflexivity. }
        rewrite H''.
        reflexivity.
      }
    rewrite H'.
...
Proof.
  ...
  - rewrite <- mult_n_Sm. simpl. rewrite add_comm. rewrite H. reflexivity.
Qed.
...
Proof.
  intro b.
  assert (forall n, n * 2 + 1 = S(n * 2)). { intro n. rewrite add_comm. rewrite plus_1_l. reflexivity. }
  simpl.
  induction b.
  - reflexivity.
  - simpl. rewrite H.  reflexivity.
...
(* FILL IN HERE *)
Example normalize0 : normalize(B0 Z) = Z.
simpl. reflexivity. Qed.

Example normalize1 : normalize(B0 (B0 (B0 Z))) = Z.
simpl. reflexivity. Qed.

Example normalize2 : normalize(B0 (B1 (B0 (B0 Z)))) = B0(B1 Z).
simpl. reflexivity. Qed.
...
Proof.
  assert (forall n : nat, nat_to_bin (double n) = double_bin (nat_to_bin n)).
    {
      intro n. induction n. 
      * reflexivity.
  ...
  - 
  {
    replace (bin_to_nat (B0 b)) with (double (bin_to_nat b)).
    + 
    {
      simpl. rewrite <- IHb. 
    ...
    +
    {
        simpl. rewrite double_plus. simpl. 
        assert (forall n : nat, n + n = n * 2). 
        {
          intro n.
          induction n.
          * reflexivity.
  ...
  -
  {
    simpl.
    rewrite <- IHb.
    replace (bin_to_nat b * 2 + 1) with (S(double (bin_to_nat b))).
    +
    {
      simpl. 
      rewrite H.
      assert (forall b : bin, incr (double_bin b) = B1 b).
        {
          intro b0. simpl. 
          induction b0.
          - reflexivity.
    ...
    +
    {
      assert (forall n : nat, S(double n) = n * 2 + 1).
        {
          intro n.
          induction n.
          * reflexivity.
          * rewrite double_incr. rewrite IHn. reflexivity. 
        }
      rewrite H0. reflexivity.
    }
  }

...
(* Definition sum (s1 s2 : bag) : bag := s1 ++ s2. *)
...
Fixpoint included (s1 : bag) (s2 : bag) : bool :=
  match s1 with 
  | nil      => true
  | n :: s1' => match eqb (count n s2) 0 with
                | true => false
                | false => included s1' (remove_one n s2)
                end
  end.
...
Fixpoint eqblist (l1 l2 : natlist) : bool :=
  ...
  | n1::l1', nil => false
  | nil, n2::l2'=> false
  | n1::l1', n2::l2' => andb (eqb n1 n2) (eqblist l1' l2')
  end.
...
Proof.
  intros d x v.
  simpl.
  assert(forall x, eqb_id x x = true). { intro x0. rewrite eqb_id_refl. reflexivity. }
  rewrite H.
  reflexivity.
Qed.
(** [] *)
